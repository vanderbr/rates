...
def write_manifest(archive_dir: Path, entries: list[ArchiveEntry]) -> None:
    ...
    write_text_atomic(index_path, render_index(entries, preserved_rows))


def read_preserved_index_rows(index_path: Path) -> list[IndexRow]:
    try:
    ...
    for line in text.splitlines():
        parsed = parse_index_table_row(line)
        if parsed is None:
            continue
        period, rates_covered, revenue_ruling, link = parsed
        if link.find("](by-year/") >= 0:
            continue
        rows.append(
            IndexRow(
                period=period,
                rates_covered=rates_covered,
...
def parse_index_table_row(line: str) -> tuple[str, str, str, str] | None:
    stripped = line.strip()
...
def render_index(
    ...
    preserved_rows: list[IndexRow] | None = None,
) -> str:
    lines = [
    ...
    for row in index_rows(entries, preserved_rows or []):
        lines.append(
            "| "
            + " | ".join(
                [
...
def index_rows(
    ...
    rows = [
        IndexRow(
            period=", ".join(entry.periods),
            rates_covered=", ".join(format_subject(subject) for subject in entry.subjects),
            revenue_ruling=entry.title,
            link=f"[PDF]({entry.path})",
        )
        for entry in entries
    ]
    rows.extend(row for row in preserved_rows if row.period not in archived_periods)
